...
function Cart() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:4005/login/verify")
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  const handleDecrement = (productId) => {
    setData((prevData) => {
      const item = prevData.find((item) => item.productId === productId);
      
      // Check if the count is 1
      if (item.count === 1) {
        return prevData; // Return the previous state without changes
      }
  
      // Otherwise, dispatch the action
      dispatch(decrementquantity({ userId, productId }))
        .then(() => {
          setData((prevData) =>
            prevData.map((item) =>
